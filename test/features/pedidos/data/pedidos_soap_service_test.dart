...
void main() {
  ...
  test('historial, consulta individual y cancelación usan contrato Android', () async {
    final endpoint = Uri.parse('http://localhost/ws/pedidos.asmx');
    final service = PedidosSoapService(
      endpoint: endpoint,
      soapService: SoapService(
        httpClient: SoapHttpClient(
          client: MockClient((request) async {
            ...
            if (request.body.contains('<getPedidos ')) {
              method = 'getPedidos';
              data = pedidoJson;
              expect(request.body, contains('<_intIdCliente>12</_intIdCliente>'));
            } else if (request.body.contains('<getUnPedido ')) {
              method = 'getUnPedido';
              data = seguimientoJson;
              message = 'OKPED';
              expect(request.body, contains('<_intIdPedido>321</_intIdPedido>'));
            } else {
